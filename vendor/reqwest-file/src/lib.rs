...
type RequestStream = BoxStream<'static, Result<Bytes, IoError>>;
type StreamData = (Option<u64>, RequestStream);
type RequestStreamFuture = BoxFuture<'static, Result<StreamData, IoError>>;
...
fn send_request(request: &RequestBuilder, offset: u64, size: Option<u64>) -> RequestStreamFuture {
    // note: this is to deal with renterd range parsing behaviour
    // renterd does not handle open ended range requests, always setting an end fixes this
    ...
    RequestFileProjection<'_, FF_WINDOW, FF_BUFFER>
{
    ...
    fn drive_initial(&mut self) {
        if let State::Initial = self.state {
            let future = Box::pin(send_request(self.request, *self.position, *self.size_));
